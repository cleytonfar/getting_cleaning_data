...
download.file(url = "https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip",
              destfile = "./final_project/final_project.zip",
              method = "curl")

## UNZIPPING THE FILE:
unzip(zipfile = "./final_project/final_project.zip", exdir = "./final_project/")


## 1. Merges the training and the test sets to create one data set.

## READING THE INDEPENDENT VARIABLES FROM TRAINING AND TEST SET:
foo_X_train <- fread("./final_project/UCI HAR Dataset/train/X_train.txt")
foo_X_test <- fread("./final_project/UCI HAR Dataset/test/X_test.txt")

## READING THE DEPENDENT VARIABLE FROM TRAINING AND TEST SET:
foo_Y_train <- fread("./final_project/UCI HAR Dataset/train/y_train.txt")
foo_Y_test <- fread("./final_project/UCI HAR Dataset/test/y_test.txt")
...
temp <- fread("./final_project/UCI HAR Dataset/features.txt")
...
activity_labels <- fread("./final_project/UCI HAR Dataset/activity_labels.txt")
...
subject_train <- fread("./final_project/UCI HAR Dataset/train/subject_train.txt")
subject_test <- fread("./final_project/UCI HAR Dataset/test/subject_test.txt")
